...
def InstallUSD(context):
    ...
    cmakeArgs = [
        ...
        '-DTBB_tbb_LIBRARY_DEBUG="{}"'.format(
            os.path.join(context.tbb_location, "lib", "libtbb_debug.so")
        ),
        '-DTBB_tbb_LIBRARY_RELEASE="{}"'.format(
            os.path.join(context.tbb_location, "lib", "libtbb.so")
        ),
        # Boost
        '-DBoost_INCLUDE_DIR="{}"'.format(
            os.path.join(context.boost_location, "include")
        ),
        '-DBoost_LIBRARY_DIR_DEBUG="{}"'.format(
            os.path.join(context.boost_location, "lib")
        ),
        '-DBoost_LIBRARY_DIR_RELEASE="{}"'.format(
            os.path.join(context.boost_location, "lib")
        ),
        # Glew
        '-DGLEW_INCLUDE_DIR="{}"'.format(
            os.path.join(context.glew_location, "include")
        ),
        '-DGLEW_LIBRARY="{}"'.format(
            os.path.join(context.glew_location, "lib64", "libGLEW.so")
        ),
        # OpenSubdiv
        '-DOPENSUBDIV_INCLUDE_DIR="{}"'.format(
            os.path.join(context.opensubdiv_location, "include")
        ),
        '-DOPENSUBDIV_OSDCPU_LIBRARY="{}"'.format(
            os.path.join(context.opensubdiv_location, "lib", "libosdCPU.so")
        ),
        '-DOPENSUBDIV_OSDGPU_LIBRARY="{}"'.format(
            os.path.join(context.opensubdiv_location, "lib", "libosdGPU.so")
        ),
